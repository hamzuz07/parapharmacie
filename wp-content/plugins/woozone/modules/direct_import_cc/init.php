<?php
/*
* Define class WooZoneDirectImportCCOptions
* Make sure you skip down to the end of this file, as there are a few
* lines of code that are very important.
*/
!defined('ABSPATH') and exit;
	  
if (class_exists('WooZoneDirectImportCCOptions') != true) {
	class WooZoneDirectImportCCOptions
	{
		/*
		...
		*/
		...
		static public $chrome_extension = 'https://chrome.google.com/webstore/detail/gmpiiinlandbgcfejoeaodgpfkdjnolm';
		...
		*/
		public function __construct()
		{
			...
			$this->module_folder = $this->the_plugin->cfg['paths']['plugin_dir_url'] . 'modules/direct_import_cc/';
			$this->module = $this->the_plugin->cfg['modules']['direct_import_cc'];
			...
			// not used - verification on 2021-07-15
			//add_action( 'wp_ajax_WooZoneDirectImportCCOptionsAuth', array( $this, 'auth_request' ) );
		}
	    ...
	    */
		protected function _registerAdminPages()
    	{
    		add_submenu_page(
    			...
    			$this->the_plugin->alias . " " . __('Direct Import CC', $this->the_plugin->localizationName),
	            __('Direct Import CC', $this->the_plugin->localizationName),
	            'manage_options',
	            $this->the_plugin->alias . "_direct_import_cc",
...
?>
		<div id="<?php echo WooZone()->alias?>" class="WooZone-direct-import">
			...
			<div class="<?php echo WooZone()->alias?>-content"> 
				...
				<?php
				// show the top menu
				WooZoneAdminMenu::getInstance()->make_active('import|direct_import_cc')->show_menu();
				?>
				...
				<section class="WooZone-main">
					...
					echo WooZone()->print_section_header(
						$this->module['direct_import_cc']['menu']['title'],
						$this->module['direct_import_cc']['description'],
						$this->module['direct_import_cc']['help']['url']
					);
...
<?php
					...
					'direct_import_cc' => array(
						...
						'elements' => array(
							...
							'headeline' => array(
								...
								'html' => '
									<div class="WooZone-direct-import-notice">
										...
										<br/><br/>
										<a target="_blank" href="' . self::$chrome_extension . '">Get the WZone Direct Import Extension here </a></p>
									</div>
									'
							),
							...
							'api_secret' => array(
								...
								'_extra_authpage_id' => 'WooZone-direct-import-auth-page-cc',
							),
						)
					)
				)
		...
		* Singleton pattern
		*
		* @return WooZoneDirectImportCCOptions Singleton instance
		*/
		...
		public function auth_request()
		{
			...
			if (1) {
				...
				$direct_import_option = get_option( $this->the_plugin->alias . '_direct_import_cc', true );
				...
				update_option( $this->the_plugin->alias . '_direct_import_cc_access_token', $access_token );
				update_option( $this->the_plugin->alias . '_direct_import_cc_login', $login );
...
// Initialize the WooZoneDirectImportCCOptions class
$WooZoneDirectImportCCOptions = WooZoneDirectImportCCOptions::getInstance();
