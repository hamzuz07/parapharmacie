...
//		);
	...
	public function search_results() {
		...
		if ( !class_exists('WooCommerce') ) $search_options = array_diff($search_options, array('products', 'product'));
		...
		foreach ( $search_options as $key => $value ) {
			...
			if ( in_array($value, array('products', 'product')) ) {
				...
				$value = str_replace(array('posts', 'pages', 'portfolio'), array('post', 'page', 'etheme_portfolio'), $value);
...
//				}
				...
				$product_args = array(
					...
					'all_count' => isset($post_types_counts['products']) ? $post_types_counts['products'] : $post_types_counts['product'],
//					'date'  => '',
				);
				...
				if ( (apply_filters('etheme_ajax_search_products_sku', false) || $is_shortcode) && wc_product_sku_enabled() && ( $product->get_sku() || $product->get_type() == 'variable' ) ) {
