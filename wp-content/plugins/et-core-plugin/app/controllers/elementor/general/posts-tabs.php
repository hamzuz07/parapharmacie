...
//				$this->add_render_attribute( 'tab-mobile', 'class', 'etheme-elementor-tab-active' );
            ...
            if ( $posts && $posts->have_posts() ) {
                ...
                <div <?php $this->print_render_attribute_string( 'wrapper' ); ?>>
                    ...
                    if ( $settings['navigation'] == 'pagination' && !!!$settings['navigation_pagination_ajax'] ) {
                        ...
                        $new_limit = $item_settings['limit'] != -1 ? $item_settings['limit'] : $posts->found_posts;
                        if ( $page > 1 ) {
		                    ...
		                    switch ($settings['navigation']) {
			                    ...
			                    case 'pagination':
				                    ...
				                    $right_arrow = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" viewBox="0 0 24 24">' .
				                                   ...
				                                   '</svg>';
				                    $total = $posts->found_posts >= $item_settings['limit'] ? ceil($item_settings['limit']/$item_settings['posts_per_page']) : $posts->max_num_pages;
                                    if ( $posts->found_posts >= $item_settings['limit'] && $item_settings['limit'] == -1)
                                        $total = ceil($posts->found_posts/$item_settings['posts_per_page']);
	...
	public function get_content_post($settings) {
		...
		foreach (self::get_post_elements() as $key => $string_text) {
			...
			switch ($key) {
				...
				case 'categories':
				    ob_start();
                    $this->render_post_categories_tags();
                    $post_content[$key] = ob_get_clean();
					break;
				case 'tags':
				    ob_start();
                    $this->render_post_categories_tags('post_tag');
                    $post_content[$key] = ob_get_clean();
					break;
				case 'button':
				    ob_start();
	...
	protected function render_post_categories_tags($type = 'category') {
		$id = get_the_ID();
		switch ($type) {
			case 'post_tag':
