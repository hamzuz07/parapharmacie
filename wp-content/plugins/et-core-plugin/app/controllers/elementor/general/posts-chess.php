		...
		if ( $posts && $posts->have_posts() ) {
            ...
            <div <?php $this->print_render_attribute_string( 'wrapper' ); ?>>
                ...
                if ( $settings['navigation'] == 'pagination' && !!!$settings['navigation_pagination_ajax'] ) {
                    ...
                    $new_limit = $settings['limit'] != -1 ? $settings['limit'] : $posts->found_posts;
                    if ( $page > 1 ) {
                    ...
                    switch ($settings['navigation']) {
                        ...
                        case 'pagination':
                            ...
                            $right_arrow = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" viewBox="0 0 24 24">' .
                                           ...
                                           '</svg>';
                            $total = $posts->found_posts >= $settings['limit'] ? ceil($settings['limit']/$settings['posts_per_page']) : $posts->max_num_pages;
                            if ( $posts->found_posts >= $settings['limit'] && $settings['limit'] == -1)
                                $total = ceil($posts->found_posts/$settings['posts_per_page']);
